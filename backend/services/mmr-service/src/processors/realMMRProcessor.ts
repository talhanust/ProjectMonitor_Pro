...
export class RealMMRProcessor {
  ...
  private calculateConfidence(data: any): number {
    ...
    if (data.projectId !== 'UNKNOWN') {
      score += 20;
    }
    checks += 20;

    if (data.summary?.totalBudget > 0) {
      score += 20;
    }
    checks += 20;

    if (data.summary?.physicalProgress > 0) {
      score += 20;
    }
    checks += 20;

    if (Object.keys(data.annexures || {}).length > 0) {
      score += 20;
    }
    checks += 20;

    if (data.month !== 'Unknown') {
      score += 20;
    }
    checks += 20;
