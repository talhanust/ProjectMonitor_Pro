...
function updatePackageJson() {
  ...
  try {
    ...
    // Merge scripts (preserving existing ones that aren't being updated)
    packageJson.scripts = {
      ...packageJson.scripts,
      ...scriptsToAdd
    };
    ...
    console.log('✅ Successfully updated package.json with comprehensive scripts');
    ...
    console.log('\n🚀 You can now use:');
    console.log('  npm run check:all    - Run all checks (lint, typecheck, tests)');
    console.log('  npm run dev          - Start development servers');
    console.log('  npm run build:all    - Build all packages');
    console.log('  npm run fresh        - Clean install and rebuild everything');
...
// Also create workspace-specific package.json updates
function updateWorkspacePackageJsons() {
  ...
  workspaces.forEach(workspace => {
    const fullPath = path.join(process.cwd(), workspace.path);
    if (fs.existsSync(fullPath)) {
      try {
        ...
        console.log(`✅ Updated ${workspace.path}`);
      } catch (error) {
        console.log(`⚠️ Could not update ${workspace.path}: ${error.message}`);
      }
    }
  });
}
...
// Run the updates
console.log('🔧 Updating package.json files with comprehensive scripts...\n');
updatePackageJson();
console.log('\n📦 Updating workspace package.json files...\n');
updateWorkspacePackageJsons();

console.log('\n✨ Setup complete! Your monorepo now has a comprehensive set of scripts.');
console.log('\n💡 Tip: You may need to install additional dependencies:');
console.log('  npm install -D concurrently rimraf tsx husky lint-staged');
